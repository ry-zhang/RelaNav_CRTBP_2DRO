...
function [structRss, structState, distance] = ...
   EPM_CD_EKF_func(P0, Cerr, SC1, SC2, rm, simdur, simstep, sigma_range, bias_range)
...
% 模拟测量数据的噪声（m）
sigma_range = sigma_range/LU;  %range measurement in LU
...
% [tX_A_MCR_Eph,tX_AB_MCLVLH_Eph]=ReadOrbit(files_A,files_AB);
% 
...
% load('XX_A_MCR_Eph_15d.mat','XX_A_MCR_Eph');
load('XX_A_MCR_Eph_70d');
XX_A_MCR_Eph=xx_MCR_target;
load('XX_AB_MCLVLH_Eph');
...
n_obs = daysnum*86400/simstep;
% n_obs = size(XX_A_MCR_Eph,1)/(step/60);%与输入轨道相关

%主星（参考星）
Sc_1st=SC1;%dro

%CRTBP线性化相对动力学模型 分解出的特征向量和特征值
...
% x0_REL_true=x0_REL;
% x0_REL(1:3)=x0_REL(1:3)*LU;
% x0_REL(4:6)=x0_REL(4:6)*SU;
% x0_REL_true(1:3)=x0_REL_true(1:3)*LU;
% x0_REL_true(4:6)=x0_REL_true(4:6)*SU;

t0_jd=2.459945500800764e+06;
x0_A_eci=[2.812783457165156e+05,2.693296911464951e+05,1.111300923290306e+05,0.380336688813009,0.801265448424371,0.420692542988196];
...
%     Y0_rel_1=[0,0.4322 ,0,0.8789,0,0]'*10^-05;
...
%     Y0_rel_6=[0,0,-0.524933677581048,0,0,0]'*10^-05;
    ...
    Y0_rel_5=x0_REL(5,:)';%法向1ZHANG
    Y0_rel_6=x0_REL(6,:)';%法向2
% 
    C1=20000/ Y0_rel_1(2)/LU;
    C2=20000/ Y0_rel_2(1)/LU;
    C3=20000/ Y0_rel_3(2)/LU;
    C4=20000/ Y0_rel_4(1)/LU;
    C5=10^-5;
    C6=20000/ Y0_rel_6(3)/LU;


    C0_true=[0 0  C3 0 0 0]%模态系数真值,尺度统计包含在内
    Y0_true=(C0_true*x0_REL)';%相对状态真值
    YL_true_0 = [IC(Sc_1st,1:3),IC(Sc_1st,4:6)]';%dro主星初值 
    C0_true=C0_true';
    x0_REL_0=x0_REL;

%     Funfit=funfit(YL_true_0,x0_REL_0);
%     save('Funfit.mat','Funfit');

%     %%0时刻时间对齐,统一到星历时间
%     [xA,r0_MCRLVLH_rel_CRTBP,Sia]=TimeAlignmentSia(C0_true,YL_true_0,x0_REL_0,XX_A_MCR_Eph(1,:))
%     YL_true=xA;
%     Y0_true=r0_MCRLVLH_rel_CRTBP;
%     x0_REL=Sia';
%     x0_REL_true=x0_REL;
% %     YL_true=[XX_A_MCR_Eph(1,1:3)./LU,XX_A_MCR_Eph(1,4:6)./SU]';

    %%0时刻时间对齐,统一到星历时间
    [xA,r0_MCRLVLH_rel_CRTBP,Sia]=TimeAlignmentSiaFit(C0_true,XX_A_MCR_Eph(1,:))
    YL_true=xA;
    Y0_true=r0_MCRLVLH_rel_CRTBP;
%     x0_REL=Sia';
%     x0_REL_true=x0_REL;
...
%     % 主星轨道和相对轨道
%     [Y2,Y1,Sia_true]=LT(C0_true,YL_true_CRTBP,x0_REL_true_old,tspan,t_CRTBP);
%     Y0_true=Y1;
%     YL_true=Y2;
%     x0_REL_true=Sia_true';
       
   YL_true=[XX_A_MCR_Eph(i+1,1:3)./LU,XX_A_MCR_Eph(i+1,4:6)./SU]';%t(i+1)真实星历轨迹
   Y0_true=[XX_AB_MCLVLH_Eph(i+1,1:3)./LU,XX_AB_MCLVLH_Eph(i+1,4:6)./SU]';

%      %i+1时间对齐,统一到星历时间
%      [YL,r0_MCRLVLH_rel_CRTBP,Sia]=TimeAlignmentSiaFit(C0_true,XX_A_MCR_Eph(i+1,:));
%      YL_true=YL;%crtbp
%       Y0_true= r0_MCRLVLH_rel_CRTBP;


%      Y0_true1=r0_MCRLVLH_rel_CRTBP;
%      x0_REL_true=Sia';
% 
%      YL_true1-YL_true
%      Y0_true1-Y0_true
...
%      res= YL_true-YL_true1
%      res1=Y0_true-Y0_true1
...
%     x0_REL=Sia';
     ...
     [YL,r_MCRLVLH_rel_CRTBP,Sia]=TimeAlignmentSiaFit(C,XX_A_MCR_Eph(i+1,:));
%      YL_true=YL;
     Y= r_MCRLVLH_rel_CRTBP;
...
hold on;
plot3(trueState(1,1)*LU,trueState(1,2)*LU,trueState(1,3)*LU,'g^');
hold on;
plot3(trueState(end,1)*LU,trueState(end,2)*LU,trueState(end,3)*LU,'r*');
hold on;

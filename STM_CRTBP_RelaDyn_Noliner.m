function A=STM_CRTBP_RelaDyn(t0, tf,x, mu)
...
%set up the initial condition, y0 for 
y0=0;
...
options=odeset('RelTol',1e-16,'AbsTol',1e-22);             %set tolerences
[t,Y]=ode113(@(t,x)sysSolveCRTBP_RelaDyn_Noliner(t,x,mu),tspan, y0,options);   %integrate the system
%[t,Y]=ode113('sysSolveCRTBP_RelaDyn',tspan, y0,options,[],mu); 
